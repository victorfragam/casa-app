...
// ══════════════════════════════════════════════
// FONTS  ← a raiz do bug: famílias com espaço PRECISAM de aspas no inline style,
// senão o React emite `font-family:Baloo 2` (inválido) e o navegador cai no serif.
// ══════════════════════════════════════════════
const F = {
...
// gradientes cheios (badges de conquista, avatares)
const GRADS = {
...
const MKTDATA = [
  ...
  { id:"folga",  name:"Folga da louça (1 semana)", icon:"restaurant",       th:"violet", cost:350, tier:"Alto"  },
...
function TaskIcon({ task, size=44 }) {
  ...
  return (
    <div style={{width:size,height:size,borderRadius:Math.round(size*.3),background:th.bg,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 3px 0 ${th.sd}`,flexShrink:0}}>
      <Icon name={task.icon} size={Math.round(size*.54)} color={th.fg}/>
    </div>
  )
}
// moeda dourada (substitui todo savings/🪙)
...
function TaskMeta({ task }) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,fontFamily:F.body,fontWeight:800,fontSize:11.5,color:C.soft}}>
      ...
      <PtsBolt n={task.pts} size={13}/>
      <Coin n={task.coins} size={13}/>
    </div>
  )
}
...
function Card({ children, style={} }) {
  return <div style={{...C.card,padding:18,...style}}>{children}</div>
}
...
function Toast({ msg, onClose }) {
  ...
  return (
    <div style={{position:"fixed",bottom:104,left:"50%",transform:"translateX(-50%)",
      background:"#211B33",color:"#fff",borderRadius:14,padding:"12px 20px",
      fontFamily:F.display,fontWeight:700,fontSize:15,zIndex:999,
      boxShadow:"0 8px 24px rgba(0,0,0,.25)",whiteSpace:"nowrap",animation:"toastIn .3s ease-out"}}>
      {msg}
    </div>
  )
}
...
function Onboarding({ onDone }) {
  ...
  const inp = { width:"100%", padding:"15px 16px", borderRadius:16, border:"2px solid #E3DDF1",
    fontFamily:F.body, fontWeight:700, fontSize:16, color:C.text, outline:"none", boxSizing:"border-box", background:"#fff" }
  const fieldLabel = { fontFamily:F.body, fontSize:12.5, fontWeight:800, color:"#6E6688", margin:"0 0 7px 4px" }
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      {step===0 && (
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 28px",textAlign:"center"}}>
          ...
          <div style={{fontFamily:F.body,fontWeight:700,fontSize:16,color:"#6E6688",marginTop:8,maxWidth:280,lineHeight:1.4}}>
            Transforme as tarefas da casa numa competição divertida entre vocês dois.
          </div>
          <div style={{display:"flex",gap:18,margin:"34px 0"}}>
            {[["emoji_events","Ranking",C.violet],["paid","Recompensas",C.gold],["local_fire_department","Sequências",C.fire]].map(([icon,label,col])=>(
              <div key={label} style={{textAlign:"center"}}>
                <div style={{width:54,height:54,borderRadius:16,background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 3px 0 rgba(0,0,0,.06)",marginBottom:6}}>
                  <Icon name={icon} size={28} color={col}/>
                </div>
      ...
      {step===1 && (
        <div style={{flex:1,padding:24,display:"flex",flexDirection:"column"}}>
          ...
          <div style={fieldLabel}>NOME DA CASA</div>
          <input placeholder="Ex: Casa do Victor & Ingrid" value={house} onChange={e=>setHouse(e.target.value)} style={{...inp,marginBottom:18}}/>

          <div style={fieldLabel}>VOCÊ</div>
          <div style={{display:"flex",gap:11,alignItems:"center",marginBottom:18}}>
            <div style={{width:50,height:50,borderRadius:15,background:ME_GRAD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 4px 0 ${ME_SD}`}}>
              <span style={{fontFamily:F.display,fontWeight:800,fontSize:22,color:"#fff"}}>{ini(myName)}</span>
            </div>
            <input placeholder="Seu nome" value={myName} onChange={e=>setMyName(e.target.value)} style={inp}/>
          </div>

          <div style={fieldLabel}>SEU PAR</div>
          <div style={{display:"flex",gap:11,alignItems:"center"}}>
            <div style={{width:50,height:50,borderRadius:15,background:PAIR_GRAD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:`0 4px 0 ${PAIR_SD}`}}>
              <span style={{fontFamily:F.display,fontWeight:800,fontSize:22,color:"#fff"}}>{ini(pairName)}</span>
            </div>
            <input placeholder="Nome do seu par" value={pairName} onChange={e=>setPairName(e.target.value)} style={inp}/>
          </div>
            ...
            {LIB.map(task=>{
              ...
              return (
                <div key={task.id} style={{...C.card,padding:"13px 16px",display:"flex",alignItems:"center",gap:12}}>
                  ...
                  {n===0 ? (
                    <button onClick={()=>setAssigns({...assigns,[task.id]:1})} style={{background:"none",color:C.violet,border:`2px solid ${C.violet}`,borderRadius:12,padding:"7px 12px",fontFamily:F.display,fontWeight:800,fontSize:13,cursor:"pointer",whiteSpace:"nowrap"}}>
                      + Assumir
                    </button>
                  ) : (
...
// ══════════════════════════════════════════════
// HOME  (Direção A — header limpo, sem hero azul)
// ══════════════════════════════════════════════
function HomeScreen({ state, dispatch }) {
  ...
  return (
    <div style={{padding:"calc(env(safe-area-inset-top,20px) + 18px) 16px 100px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <Card>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
            ...
            <div style={{textAlign:"right",display:"flex",flexDirection:"column",gap:3,alignItems:"flex-end"}}>
              <PtsBolt n={`+${nextTaskData.pts}`} size={16}/>
              <Coin n={`+${nextTaskData.coins}`} size={16}/>
            </div>
          </div>
      ...
      {/* Destaque — card dourado com shine */}
...
function DraftScreen({ state, dispatch }) {
  ...
  const ptsMax=Math.max(pts0,pts1,1)
  const balanced=Math.abs(pts0-pts1)<=40
  ...
  return (
    <div style={{padding:"calc(env(safe-area-inset-top,20px) + 18px) 16px 140px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Card>
        ...
        {[{name:state.users[0].name,pts:pts0,grad:ME_GRAD,trk:"#EEF4FF",txt:"#2E7BF0",ini:ini(state.users[0].name)},
          {name:state.users[1].name,pts:pts1,grad:PAIR_GRAD,trk:"#FFEAF3",txt:"#FF4F9A",ini:ini(state.users[1].name)}].map((r,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:i===0?7:0}}>
            <div style={{width:26,height:26,borderRadius:8,background:r.grad,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontFamily:F.display,fontWeight:800,fontSize:13,color:"#fff"}}>{r.ini}</span>
            </div>
      ...
      {/* Toggle de usuário (drafт no aparelho compartilhado) */}
...
function TasksScreen({ state, dispatch }) {
  ...
  const history=(state.history||[]).filter(h=>h.userId==="0").slice(-6).reverse()
  ...
  return (
    <div style={{padding:"calc(env(safe-area-inset-top,20px) + 18px) 16px 100px",display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div key={assign.id} style={{display:"flex",alignItems:"center",gap:11,padding:"13px 0",borderBottom:i<myAssigns.length-1?"1px solid #F2EEFA":"none"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {tTotal>1?(
                    ...
                    <div style={{fontFamily:F.body,fontSize:11.5,fontWeight:800,color:isOk?C.green:C.soft,display:"flex",alignItems:"center",gap:4}}>{isOk?"Concluída":<>0 / 1 · <PtsBolt n={`+${task.pts}`} size={13}/></>}</div>
                  )}
                </div>
                <button onClick={()=>{ if(!isOk){ if(navigator.vibrate)navigator.vibrate(8); dispatch({type:"COMPLETE",userId:"0",taskId:assign.id}) }}} disabled={isOk}
                  style={{width:38,height:38,borderRadius:"50%",border:"none",cursor:isOk?"default":"pointer",background:isOk?C.green:C.violet,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:isOk?"0 3px 0 #1FA85B":`0 3px 0 ${C.violetSd}`,flexShrink:0}}>
      ...
      {history.length>0&&(
        <>
          <div style={{fontFamily:F.body,fontWeight:800,fontSize:12,color:C.soft,letterSpacing:.5,textTransform:"uppercase",margin:"0 4px"}}>Histórico recente</div>
          <div style={{...C.card,padding:"4px 14px"}}>
            {history.map((h,i)=>(
...
function MarketScreen({ state, dispatch, showToast }) {
  ...
  const handle=(r)=>{ if(coins<r.cost) return; if(navigator.vibrate)navigator.vibrate(12); dispatch({type:"REDEEM",userId:"0",rewardId:r.id}); showToast(`🎁 ${r.name} resgatado!`) }
      ...
      {nextReward&&(
        <div style={{background:"linear-gradient(135deg,#FF7DB5,#FF4F9A)",borderRadius:22,padding:16,color:"#fff",boxShadow:"0 4px 0 #E02E7D, 0 10px 24px rgba(255,79,154,.22)",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:13,marginTop:8,position:"relative"}}>
            <div style={{width:52,height:52,borderRadius:16,background:"rgba(255,255,255,.22)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <Icon name={nextReward.icon} size={30} color="#fff"/>
            </div>
            <div style={{flex:1}}>
          ...
          <div style={{marginTop:12,height:10,borderRadius:999,background:"rgba(0,0,0,.18)",overflow:"hidden",position:"relative"}}>
        ...
        return (
          <div key={tier}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {items.map(r=>{
                const th=TH[r.th], can=coins>=r.cost
                return (
                  <div key={r.id} style={{...C.card,padding:14,textAlign:"center"}}>
                    ...
                    {can?(
                      <button onClick={()=>handle(r)} style={{width:"100%",background:C.violet,color:"#fff",border:"none",borderRadius:12,padding:9,fontFamily:F.display,fontWeight:800,fontSize:13.5,boxShadow:`0 3px 0 ${C.violetSd}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
...
// ══════════════════════════════════════════════
// PROFILE  (card compacto + anel — sem hero azul)
// ══════════════════════════════════════════════
function ProfileScreen({ state, dispatch }) {
  ...
  return (
    <div style={{padding:"calc(env(safe-area-inset-top,20px) + 18px) 16px 100px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{textAlign:"center",fontFamily:F.body,fontSize:11,fontWeight:700,color:C.softer,padding:"0 8px"}}>{state.house.name} · protótipo local (cada um terá login próprio na versão final)</div>
    </div>
  )
}
  ...
  switch(action.type) {
    ...
    case "COMPLETE": {
      ...
      if (!bonusGiven) {
        ...
        if (mine.length>0 && mine.every(a=>(a.done||0)>=a.count)) { s0.coins+=50; bonusGiven=true; gotBonus=true }
      }
      const now = new Date()
      ...
      return {
        ...
        _newAchs: newAchs,
        _reward: { pts:task.pts, coins:earned, bonus:gotBonus },
      }
    }
    case "REDEEM": {
  ...
  const dispatch = useCallback((action) => {
    setState(prev => {
      ...
      if (next._reward) {
        ...
        if (r.bonus) { setToast(null); setTimeout(()=>setToast("Draft completo! +50 moedas 🎉"),60) }
      }
